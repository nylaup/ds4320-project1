...
#### Press Release 
[How Many Ambulances Do We Need? Predicting Ambulance Demand](https://github.com/nylaup/ds4320-project1/blob/main/PressRelease.md)    
...
| :--- | :--- | :--- |
| Artificial intelligence for modeling and <br> understanding extreme weather and <br> climate events | Research on using AI to identify severe <br> weather events to predict locations <br> needing ambulances | [Link](https://myuva-my.sharepoint.com/:b:/g/personal/mge9dn_virginia_edu/IQBSZuiUUJAuRYwdIl98_0kqAf72h6G0nVX-b5mcgf2kcqA?e=DTRNls)
| On the optimization of heterogeneous <br> ambulance fleet allocations | Proposing an algorithm for optimal <br> allocation of of response vehicle types <br> to improve patient outcomes | [Link](https://myuva-my.sharepoint.com/:b:/g/personal/mge9dn_virginia_edu/IQAKCArBgBAaTolwgE5aZHWqAa9NBK5ELEBcCWG0Ow9lx6Y?e=UlrMms) |
| Ambulance route optimization in a <br> mobile ambulance dispatch system <br> using deep neural network | Creating a neural network model to <br> improve ambulance routes on traffic <br> and road data | [Link](https://myuva-my.sharepoint.com/:b:/g/personal/mge9dn_virginia_edu/IQANjGln32COTq94a1Dozi4pAbtJRt9cs-fBnajuIjfbB-k?e=dsPQTH) |
| Simulation modeling and optimization <br> for ambulance allocation considering <br> spatiotemporal stochastic demand | Proposing a method that uses <br> simulations to optimize ambulance <br> allocation and EMS system processes | [Link](https://myuva-my.sharepoint.com/:b:/g/personal/mge9dn_virginia_edu/IQDJPnbsm1cLRKMYS1ESWt8vAZkS4n6wWiTPrFvjPV2TzPQ?e=m8Bygn) |
| Where will the next emergency event <br> occur? Predicting ambulance demand <br> in emergency medical services using <br> artificial intelligence | Modeling emergency events and <br> developing a neural network that <br> predicts future demand and use a <br> model to allocate ambulances | [Link](https://myuva-my.sharepoint.com/:b:/g/personal/mge9dn_virginia_edu/IQDhNEzAlXz4R5l_2YuTTddYAZT1teOvE4VKblWf1Nb1jA0?e=dfxmNQ) |
...
| :--- | :--- | :--- |
| EMS Incidents | EMS calls with information on location | [Link](https://myuva-my.sharepoint.com/:x:/g/personal/mge9dn_virginia_edu/IQAi1j0OWWujR50DTupRbxB5AWpqtZ6mcvqrZUemYF7GU5Y?e=fEQQYJ) |
...
| NYC Events | Information on scheduled events in NYC | [Link](https://myuva-my.sharepoint.com/:x:/g/personal/mge9dn_virginia_edu/IQAILrjhmHFvQKRCB7H9qu3RAeypLpTk1RNEic7X7Rgzru4?e=bfOps2) |
...
| Name | Data Type | Description | Example |      
| :--- | :--- | :--- | :--- |
| ID | Num | Unique identifier for each event | 808651 |
...
| LocationTimeID | Cat | Location, date, and hour of weather, used to join with incidents | QUEENS_2017-03-29 01:00:00 |    

EventJunction
| Name | Data Type | Description | Example |      
| :--- | :--- | :--- | :--- |
| EventJunctionID | Num | Unique identifier for event incident junction table | 2 |
| ID | Num | Foreign key used to link to events | 808651 |
| CAD_Incident_ID | Num | Foreign key used to link to incidents | 160010002 |
| LocationTimeID | Cat | Location, date, and time to ensure uniqueness | QUEENS_2017-03-29 01:00:00 |  
            
#### Uncertainty Quantification 
Quantifying uncertainty for the incidents table, there is no uncertainty in the numeric id or the zip code. Since we rounded the start datetime to the nearest hour, for all of these there is +- 59 minutes.        
For the events, the id is generated to be unique for each event so there is no uncertainty in this. Since we rounded the event start times to the nearest hour, there is +-59 minutes uncertainty.       
For the weather, since we are generalizing to the hour and one coordinate for each borough, there is some uncertainty on the exact weather for the incident's specific location and time, as well as some potential for error with weather readings, so for temperature there is an uncertainty of +-1 to 3 degrees. The weather code values are very specific to the different weather incident, but there could be slight uncertainty of +-1.       
 For the demographic information, there is the potential for inaccuracies with census data collection for most of the variables. Based on the distribution, I would guess the violent crime rate has an uncertainty of +-0.5, household mean income +-100, percent under 18 +-1, percent above 65 +-0.5, and population +-1000.    
