...
(NNU FIX) In order to solve the problem of predicting EMS calls, I had to do some judgement calls of what factors I thought might be important and would influence the model. I decided to look at demographics, weather, and events as these were all publicly accessible and made sense. In order to get a proper scope of the data, I decided to select two years to analyze. I didn't want to use recent data, as it may not yet be available for some sources. I also chose to avoid years that would be affected by the COVID-19 pandemic, as those caused irregularities in health systems. For these reasons I chose to query data form 2016 and 2017, which was enough data that it was feasible to load and process, and it also gave a sufficient amount of data for the model to learn from.             
